...
class MembershipService:
        ...
        **kwargs: Any,
        ...
        for attempt in range(retries + 1):
            try:
                ...
                wait = min(exc.retry_after, 30)
                log.warning("Flood limit for %s, sleeping %ss", chat_id, wait)
                ...
                log.debug("Cannot send to %s: %s", chat_id, exc)
                return None
        return None
    ...
    async def get_owner_id(self, chat: Chat, refresh: bool = False) -> int | None:
        ...
        try:
            admins = await self.bot.get_chat_administrators(chat.chat_id)
    ...
    async def ask_owner_about_member(self, chat: Chat, member: Member, source: str = "join") -> PendingJoin | None:
        ...
        for admin_id in recipients:
            ...
            if msg:
                delivered += 1
                await self.db.add_prompt_message(pending.id, admin_id, msg.message_id)
        if not delivered:
    ...
    async def _finalize_prompt(
        ...
        for admin_id, message_id in await self.db.prompt_messages(pending.id):
            try:
                await self.bot.edit_message_text(
                    text, chat_id=admin_id, message_id=message_id, reply_markup=None,
                    disable_web_page_preview=True,
                )
            except TelegramRetryAfter as exc:
    ...
    async def create_invite_link(
        ...
        try:
            link = await self.bot.create_chat_invite_link(
                chat.chat_id, name=label, creates_join_request=join_request
            )
        except TelegramRetryAfter as exc:
    ...
    async def sync_chat_members(self, chat: Chat) -> dict[str, int]:
        ...
        for member in await self.db.list_members(chat.chat_id, "active", limit=5000):
            result["checked"] += 1
            try:
                cm = await self.bot.get_chat_member(chat.chat_id, member.user_id)
            except TelegramRetryAfter as exc:
